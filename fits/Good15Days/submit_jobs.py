...
def write_condor_job(fn, ds, out_dir, channels, beam_type='gaussian', cuts=None):
    f=open(fn,'w')
    f.write("""
Universe        = vanilla
Executable      = /astro/u/fbianchin/Research/bmxobs/fits/Good15Days/run_fitter.py
Arguments       = -dataset %s -out_dir %s -channels %s -save_plots -beam_type %s -cuts %s
Requirements    = (CPU_Speed >= 1)
Rank            = CPU_Speed
request_memory  = 12000M
request_cpus    = 2
...
"""%(ds, out_dir, channels, beam_type, cuts, ds, channels, ds, channels, ds, channels))
    f.write ('Queue \n')
    f.close()
        ...
        for channels in channels_combo:
            for cut in cuts:
                ...
                for data_id in data_ids:
                    ...
                    write_condor_job(fname, data_id, out_dir, channels, beam_type=beam, cuts=cut)
